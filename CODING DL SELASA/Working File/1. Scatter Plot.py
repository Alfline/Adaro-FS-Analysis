...
# Create scatter plots to visualize the variables and their clusters
...
if labels_2023 is not None:
    ...
    # We're using seaborn for enhanced visualization
    sns.scatterplot(x=range(len(filtered_df)), y=X_2023.flatten(), 
                    hue=labels_2023, palette='viridis', s=150)
    plt.xticks(range(len(filtered_df)), filtered_df['Item'].values, rotation=45, ha='right')
    plt.title('Q4 2023 Variables and Clusters (Scatter Plot)')
...
if labels_2024 is not None:
    # Create a scatter plot with colors based on cluster assignment
    # We're using seaborn for enhanced visualization
    sns.scatterplot(x=range(len(filtered_df)), y=X_2024.flatten(), 
                    hue=labels_2024, palette='viridis', s=150)
    plt.xticks(range(len(filtered_df)), filtered_df['Item'].values, rotation=45, ha='right')
    plt.title('Q4 2024 Variables and Clusters (Scatter Plot)')
    plt.ylabel('Value (USD)')
    plt.xlabel('Financial Variables')
    # Format y-axis to show values in millions
    plt.ticklabel_format(style='scientific', axis='y', scilimits=(0,0))
...
# Print scatter plot cluster analysis
print("\nScatter Plot Cluster Analysis:")
print("Each point represents a financial variable, colored by its cluster assignment")
print("X-axis: Financial Variables (Revenue, Operating Income, Profit)")
print("Y-axis: Financial Values in USD")
print("Colors: Different clusters identified by K-means algorithm")

# Create a summary table showing cluster assignments
comparison_df = pd.DataFrame({
    'Variable': filtered_df['Item'].values,
    'Q4_2023_Value': X_2023.flatten(),
    'Q4_2023_Cluster': labels_2023 if labels_2023 is not None else np.nan,
    'Q4_2024_Value': X_2024.flatten(),
    'Q4_2024_Cluster': labels_2024 if labels_2024 is not None else np.nan
})
